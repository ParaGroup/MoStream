...
- Standard, cooperative, and adaptive cooperative runtime backends.
...
MoStream provides three execution runtimes: the standard runtime, the
cooperative runtime, and the adaptive cooperative runtime.
...
not supported with the cooperative or adaptive cooperative runtimes.

### Adaptive Cooperative Runtime

The adaptive cooperative runtime is selected with:

```mojo
pipeline.run_cooperative_adaptive(n_workers)
```

Like the cooperative runtime, it executes pipeline actors on a fixed number of
scheduler threads. Unlike the plain cooperative runtime, each stage can be
created with a maximum parallelism degree, while the scheduler dynamically
activates or quiets stage replicas at run time.

The adaptive scheduler monitors inter-stage queue pressure and uses it to grow
stages that appear to be bottlenecks, or lazily shrink stages that look
over-provisioned. For example:
...
pipeline.run_cooperative_adaptive(20)
```

creates up to 20 logical replicas per stage, but only 20 scheduler threads.
The source stage starts at its configured maximum parallelism, while downstream
stages are activated adaptively up to their maximum degree. This is useful for
pipelines whose bottleneck changes over time or whose best per-stage
parallelism is not known in advance.
